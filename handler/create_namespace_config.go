...
func (h *defaultHandler) CreateNamespaceConfig(ctx context.Context, req *go_block.UserRequest) (*go_block.UserResponse, error) {
	...
	errGroup.Go(func() error {
		_, err = emailRepo.Create(ctx, &go_block.Email{
			...
			BodyMessage:    fmt.Sprintf("Thank you for signing up to %s. In order to get started, we ask of you to confirm your email by entering the following numbers in your %s app.", config.Name.Body, config.Name.Body),
			FooterMessage:  fmt.Sprintf("All the best from %s team", config.Name.Body),
	...
	errGroup.Go(func() error {
		_, err = emailRepo.Create(ctx, &go_block.Email{
			...
			Logo:           config.Logo.Body,
			WelcomeMessage: "Hello",
			BodyMessage:    fmt.Sprintf("Thank you for signing up to %s. In order to reset your password, enter the following numbers in your %s app together with your new password.", config.Name.Body, config.Name.Body),
			FooterMessage:  fmt.Sprintf("All the best from %s team", config.Name.Body),
			Title:          "Verify your email",
			Subject:        "Verify your email account",
			TemplatePath:   emailVerificationTemplatePath,
