...
func New(ctx context.Context, collection *mongo.Collection, expiresAfter time.Duration) (TokenRespository, error) {
	emailNamespaceIndexModel := mongo.IndexModel{
		Keys: bson.D{
			{Key: "expires_at", Value: 1},
		},
		Options: options.Index().SetExpireAfterSeconds(int32(time.Now().Add(expiresAfter).Unix())).SetName(expiresAfterIndex),
	}
