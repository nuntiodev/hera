...
type UserRepositoryBuilder interface {
	...
	SetHasher(hash hash.Hash) UserRepositoryBuilder
...
func (ub *userRepositoryBuilder) SetHasher(hasher hash.Hash) UserRepositoryBuilder {
	ub.hasher = hasher
	return ub
}
...
func (ub *userRepositoryBuilder) Build(ctx context.Context) (user_repository.UserRepository, error) {
	...
	userRepository, err := user_repository.New(collection, crypto, ub.hasher, ub.validatePassword, ub.maxEmailVerificationAge)
	if err != nil {
		return nil, err
	}
